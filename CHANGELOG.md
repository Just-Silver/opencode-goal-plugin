...
- **TUI 里的 token 数字改为紧凑显示**：`/goal-status` 状态行与 `/goal-budget`、`/goal-resume` 的回执把预算、已用量、缓存读取、新增量由原始整数（如 `100000000`）改为 K/M/B/T 紧凑格式（`100M`、`12.3M`、`1.25K`），不再一长串 0 难辨。精度随数量级递减（缩放值 `<10` 两位、`<100` 一位、其余整数）并去掉尾随 0；整数位向下取整，避免 `999999` 显示成 `1000K`。0 与不足 1000 仍显示原值。**仅展示层**——存储、命令参数、工具返回与模型侧提示词仍是原始整数。

## [0.6.1] - 2026-09-26

### Changed

- **`/goal-status` 区分两个时间口径**：状态行由「`创建于 …` · `累计运行 …`」改为「`创建于 …（已过 …）` · `有效工作 …`」。「已过」= `now − createdAt`（自然流逝，暂停/空闲也在涨），「有效工作」= `timeUsedSeconds`（逐轮累加的活动时长，只在真跑轮时增长）——解释了「为什么暂停时耗时不动」。英文为 `created … (… ago) · active work …`。
