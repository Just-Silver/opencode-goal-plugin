...
3. 所以：**随轮次/进度变化的东西（用量计数、耗时、状态标签、时间戳…）不要进 system**。它们的出口是：工具返回（messages）、命令回执（TUI）、事件提示——都不碰 system。
...
| 谁要看 | 出口 | 进 system？ |
| --- | --- | --- |
| 模型按需 | 工具返回（tools 结果 → messages） | 否 |
| 用户查看 | 命令回执（`session.synthetic` + `resume:false` 的 `description`） | 否 |
| 事件触发 | 工具结果里的 `instruction` / 通知 | 否 |
...
- [ ] 动态信息是否有**非 system 的出口**（工具返回 / 命令回执 / 事件提示）？
