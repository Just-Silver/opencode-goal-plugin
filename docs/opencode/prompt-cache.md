...
3. 所以：**随轮次/进度变化的东西（用量计数、耗时、状态标签、时间戳…）不要进 system**。它们的出口是 messages：工具返回、命令回执、事件提示——都不碰 system。
   ⚠️ 但 **「不进 system」≠「零成本」**：工具返回与命令回执最终都会进模型上下文（命令回执经 `synthetic` 落一条消息、下一轮以 `[Synthetic context]` 出现），只是位于 system 之后的 messages 区、不击穿 system 断点。文本越长，后续每轮 token 越多。
...
| 用户查看 | 命令回执（`session.synthetic` + `resume:false`） | 否 | **是**（`text` 落历史，下轮以 `[Synthetic context]` 出现） |
| 事件触发 | 工具结果里的 `instruction` / 通知 | 否 | **是** |

> ⚠️ 「用户查看」这一行最容易误解：`resume:false` 只是**不唤醒模型**（不额外开一轮、不花一次请求），**不是**"不进模型"。**服务端插件没有"只给人看、不进模型"的出口**——回执文本一样进上下文、一样计费，还会让 messages 的 tail 断点移到它身上。要省 token 就把回执写短（`/goal-debug` 只回必要的行）。
