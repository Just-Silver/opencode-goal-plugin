...
- **确定性分支（不唤醒模型；回执走 TUI toast，不写会话消息）**：`/goal-status` 无目标 → 「本会话未设置目标。」；`/goal-budget` 空参 → 用法；`/goal-budget abc` → 「无效的预算「abc」…」；`/goal-budget 9007199254740993`（超 safe integer）→ 非法；`/goal-budget 500` 无目标 → 未设置目标。**5/5** ✓
