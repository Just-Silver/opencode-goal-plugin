...
- **每会话一条记录**：key `goal:<sessionID>`，value 为 JSON：
  ...
  - **记账时机**：每个 `session.step.ended` **增量落盘一次**（把最坏丢失窗口从「一整轮 execution」压到「一次模型调用」），**轮末**（`execution.succeeded`/`failed`）或中断再结算残留 —— 收尾轮（状态翻成 complete/blocked/budget-limited 之后仍在进行的 step）也照记。写成功后清零内存累加器，故同一段用量只写一次。对齐 codex 的 `on_tool_finish` / omp 的 `onToolCompleted`。
  - "会话文件 + 目标引用文件"合并为同一条。
...
- **2026-09-27（step 级增量落盘）**：原实现只在**轮末**（`execution.succeeded/failed/interrupted`）落账，一整轮长 execution 期间用量全悬在内存——reload/`opencode plugin update`/service 重启会**永久丢**（真机实测：一轮 40 分钟、宿主累计 ~53M，KV 仍是 0）。现改为**每个 `session.step.ended` 增量落盘一次**（对齐 codex 的 `on_tool_finish` / omp 的 `onToolCompleted`），轮末再结算残留；最坏丢失窗口降到一次模型调用。防重复靠「**写成功后才清零累加器**」（失败则保留待下个写点重试）；宿主**串行派发事件**，写点不交错。为不改动停摆语义，增量写**只记账、不判预算、不发停摆回执**——预算跃迁与回执仍由轮末的 `save` 统一处理。`pendingUsage` 随之在累加器清零后返回 `undefined`，`/goal-status` 显示收敛到持久值。
